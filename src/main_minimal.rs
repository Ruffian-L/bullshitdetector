...
fn output_results(alerts: &[BullshitAlert], format: &str) -> Result<()> {
    if format == "json" {
        ...
        // Group by severity
        let mut critical: Vec<_> = alerts.iter().filter(|a| a.severity >= 0.9).collect();
        let mut high: Vec<_> = alerts.iter().filter(|a| a.severity >= 0.75 && a.severity < 0.9).collect();
        let mut medium: Vec<_> = alerts.iter().filter(|a| a.severity < 0.75).collect();
